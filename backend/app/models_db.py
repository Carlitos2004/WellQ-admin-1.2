...
Tablas nuevas — Sistema RBAC (Roles, Permisos y Usuarios):
  - roles            → roles de acceso configurables (Super Admin, Billing, etc.)
  - permissions      → catálogo fijo de 13 permisos del sistema
  - role_permissions → tabla de unión N:M con PK compuesta (role_id, permission_id)

Campos nuevos en admin_users (RBAC):
  - admin_users.role_id      → FK nullable a roles.id; NULL = usar campo role legacy
  - admin_users.invite_token → token para flujo de invitación por email
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# TABLAS NUEVAS — SISTEMA RBAC (ROLES, PERMISOS Y USUARIOS)
# Deben definirse ANTES de AdminUser porque AdminUser referencia roles.id.
# ══════════════════════════════════════════════════════════════════════════════
