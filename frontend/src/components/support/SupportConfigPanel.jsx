...
export const SupportConfigPanel = ({ onClose, onCategoriesChanged, onRespondersChanged }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('support.errorLoadConfig'));
    } finally {
      setLoading(false);
    }
  }, [t]);
        ...
        >
          ...
          <div className="flex items-center px-8 pt-4 border-b border-wellq-gray/10 dark:border-white/5 bg-wellq-gray/3 dark:bg-white/[0.02]">
            <TabButton
              ...
              icon={Tags}
              label={t('support.tabCategories')}
            />
            <TabButton
              active={activeTab === 'responders'}
              onClick={() => setActiveTab('responders')}
              icon={Users}
              label={t('support.tabResponders')}
            />
          </div>
                              ...
                              onToggleStatus={async () => {
                                ...
                                try {
                                  if (cat.is_active) {
                                    ...
                                    toast.success(t('support.categoryDisabled'));
                                  } else {
                                    await updateTicketCategory(cat.category_id, { is_active: true });
                                    toast.success(t('support.categoryEnabled'));
                                  }
                                } catch (e) {
                                  setCategories(original);
                                  toast.error(t('support.errorChangeStatus'));
                                }
                              }}
                            />
                          </motion.div>
                        ...
                        {responders.map((resp) => (
                          <motion.div key={resp.id} variants={itemVariants}>
                            <ResponderRow
                              ...
                              onDelete={async () => {
                                if (!window.confirm(t('support.confirmDeleteResponder', { name: resp.name }))) return;
                                try {
                                  ...
                                  if (error.message.includes('409') || error.message.includes('ticket')) {
                                    ...
                                    toast.error(t('support.errorDeleteResponder'));
                                  }
                                }
                              }}
                            />
...
const CategoryCard = ({ category, onEdit, onToggleStatus, t }) => {
  let emails = [];
      ...
      <div className="flex justify-between items-start mb-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {!category.is_active && (
              <span className="px-2 py-0.5 text-[9px] font-bold bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400 rounded-md uppercase">{t('values.inactive')}</span>
            )}
          </div>
          {category.team && (
            <div className="flex items-center gap-1.5 mt-1.5 text-[11px] font-bold text-wellq-cyan uppercase tracking-wider">
              <ShieldCheck size={12} strokeWidth={2.5} />
              {t('support.team')}: {category.team}
            </div>
          )}
        </div>
...
const ResponderRow = ({ responder, onEdit, onDelete, t }) => (
  ...
  const handleEmailKeyDown = (e) => {
    if (e.key === 'Enter' || e.key === ' ' || e.key === ',') {
      ...
      if (EMAIL_RE.test(val) && !form.emails.includes(val)) {
        ...
        toast.error(t('support.invalidOrDuplicateEmail'));
      }
    }
  };
      ...
      <div className="space-y-1.5">
        ...
        <div className="w-full p-2 min-h-[46px] rounded-xl border border-wellq-gray/20 dark:border-white/10 bg-white dark:bg-black/20 flex flex-wrap gap-2 focus-within:ring-2 focus-within:ring-wellq-cyan/50">
          ...
          <input
            ...
            placeholder={form.emails.length === 0 ? t('support.notificationEmailPlaceholder') : ""}
        ...
        <p className="text-[10px] text-wellq-gray/70 font-medium pt-1"><AlertCircle size={10} className="inline mr-1" />{t('support.multiEmailHint')}</p>
      </div>
    </FormModalBase>
  );
};
...
const ResponderFormModal = ({ mode, initialData, teamOptions = [], onClose, onSaved }) => {
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.username.trim()) return toast.error(t('support.validationResponderNameUser'));
    // ── NUEVO: validar team — el backend lo requiere en CreateResponderBody ──
    if (!form.team.trim()) return toast.error(t('support.validationTeamRequired'));
    if (mode === 'create' && !form.password) return toast.error(t('support.validationPasswordRequired'));
  ...
  return (
    <FormModalBase title={mode === 'create' ? t('support.newResponder') : t('support.editResponder')} icon={Users} onClose={onClose} onSave={handleSave} saving={saving}>
