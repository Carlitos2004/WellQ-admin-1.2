...
const getTierLabel = (tier, t) => {
  const key = String(tier ?? '').toLowerCase();
  return t(`clinics.tiers.${key}`, tier ?? '');
};

const formatLastAccess = (value, locale) => {
  if (!value || value === 'waiting') return '-';
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;

  return date.toLocaleString(locale === 'es' ? 'es-CL' : 'en-US', {
    year: 'numeric',
    month: 'short',
    day: '2-digit',
    hour: '2-digit',
...
export const ClinicRow = ({
  ...
  const tierKey = String(clinic.tier ?? '').toLowerCase();
  ...
  return (
    ...
    >
      ...
      <td className="py-4 px-4 text-xs text-wellq-gray font-semibold tabular-nums">
        {formatLastAccess(clinic.lastLogin, locale)}
      </td>
      ...
      <td className="py-4 px-4">
        <div className="flex items-center gap-1 transition-opacity duration-200">
          <ActionBtn
            icon={Settings}
            title={t('clinics.actions.settings')}
